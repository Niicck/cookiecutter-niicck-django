...
class BuildRequirementsHandler:
    ...
    def set_file_dest(self):
        ...
        requirements_dir = base_dir / "build" / "requirements"
        file_name = "dev.txt" if self.is_dev else "production.txt"
        self.file_dest = requirements_dir / file_name

    def export(self):
        export_cmd = f"poetry export -f requirements.txt -o '{self.file_dest}'"
        if self.is_dev:
            export_cmd = export_cmd + " --with dev"
        process = subprocess.run(export_cmd, stderr=subprocess.STDOUT, shell=True)
